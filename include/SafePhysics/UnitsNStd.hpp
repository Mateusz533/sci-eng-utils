...
namespace Physics::Units::NStd
{
	namespace Detail
	{
		...
		template<typename T>
		concept StandardUnit = !NonStandardUnit<T> && std::is_constructible_v<T, f64> &&
							   requires(const T& instance) {
								   { T::HasNoPrefix() } -> std::same_as<bool>;
								   { instance.ToRaw() } -> Arithmetic;
								   { instance.template Cast<f64>() };
								   { instance.template Cast<f64>().ToRaw() } -> std::same_as<f64>;
								   { instance.template Cast<f64>() / instance.template Cast<f64>() } -> std::same_as<SI::Scale<f64>>;
							   };
	}

	template<Detail::StandardUnit StandardUnit, Detail::NormedFraction Scale, Detail::NormedFraction Offset, Arithmetic AccuracyType = f64>
		requires(StandardUnit::HasNoPrefix() && Scale::IsPositive() && !(Offset::IsZero() && Scale::IsIdentity()))
	...
	public:
		...
		template<Detail::StandardUnit OtherStandardUnit, Detail::NormedFraction OtherOffset, Arithmetic OtherAccuracyType>
			requires(HasSameStdUnitBase<ExtendedSibling<OtherStandardUnit, Scale, OtherOffset, OtherAccuracyType>>())
		constexpr auto operator+(const ExtendedSibling<OtherStandardUnit, Scale, OtherOffset, OtherAccuracyType>& value) const noexcept {
			using RawType = decltype(std::declval<Type>() + std::declval<OtherStandardUnit>().ToRaw());
		...
		template<Detail::StandardUnit OtherStandardUnit, Detail::NormedFraction OtherScale, Arithmetic OtherAccuracyType>
			requires(HasNoOffset())
		constexpr auto operator*(const ExtendedSibling<OtherStandardUnit, OtherScale, Detail::Fraction<0, 1>, OtherAccuracyType>& value) const noexcept {
	...
	private:
		...
		template<Detail::StandardUnit OtherStandardUnit, Detail::NormedFraction OtherScale, Detail::NormedFraction OtherOffset, Arithmetic OtherAccuracyType>
			requires(HasSameStdUnitBase<ExtendedSibling<OtherStandardUnit, OtherScale, OtherOffset, OtherAccuracyType>>())
		static constexpr Type FromOtherNStd(const ExtendedSibling<OtherStandardUnit, OtherScale, OtherOffset, OtherAccuracyType>& value) noexcept {
			if constexpr(std::is_same_v<Scale, OtherScale> && std::is_same_v<Offset, OtherOffset>) {
				return value;
			} else {
				using AccType = decltype(std::declval<AccuracyType>() * std::declval<OtherAccuracyType>());

				constexpr AccType OWN_SCALE = Scale::template ToDecimal<AccType>();
				constexpr AccType OWN_OFFSET = Offset::template ToDecimal<AccType>();

				constexpr AccType OTHER_SCALE = OtherScale::template ToDecimal<AccType>();
				constexpr AccType OTHER_OFFSET = OtherOffset::template ToDecimal<AccType>();

				constexpr AccType COMP_SCALE = OTHER_SCALE / OWN_SCALE;
				constexpr AccType COMP_OFFSET = (OTHER_OFFSET - OWN_OFFSET) / OWN_SCALE;
	...
	private:
		...
		static_assert(std::isnormal(SCALE));
		static_assert(std::isnormal(OFFSET) || OFFSET == AccuracyType{0});
...
#define GENERATE_OFFSETTABLE_NSTD_FROM_FRACTION(Name, StdType, ScNum, ScDenom, OffNum, OffDenom) \
	...
	GENERATE_NSTD_FROM_FRACTION(SquareInches, SI::SquareMeters, 9'144L * 9'144, 10'000L * 3 * 12 * 10'000 * 3 * 12)
	GENERATE_NSTD_FROM_FRACTION(SquareFeet, SI::SquareMeters, 9'144L * 9'144, 10'000L * 3 * 10'000 * 3)
	GENERATE_NSTD_FROM_FRACTION(SquareYards, SI::SquareMeters, 9'144L * 9'144, 10'000L * 10'000)
	GENERATE_NSTD_FROM_FRACTION(SquareRods, SI::SquareMeters, 9'144L * 55 * 9'144 * 55, 10'000L * 10 * 10'000 * 10)
	GENERATE_NSTD_FROM_FRACTION(SquareChains, SI::SquareMeters, 9'144L * 22 * 9'144 * 22, 10'000L * 10'000)
	...
	GENERATE_NSTD_FROM_FRACTION(CubicInches, SI::CubicMeters, 9'144L * 9'144 * 9'144, 10'000L * 3 * 12 * 10'000 * 3 * 12 * 10'000 * 3 * 12)
	GENERATE_NSTD_FROM_FRACTION(CubicFeet, SI::CubicMeters, 9'144L * 9'144 * 9'144, 10'000L * 3 * 10'000 * 3 * 10'000 * 3)
	GENERATE_NSTD_FROM_FRACTION(CubicYards, SI::CubicMeters, 9'144L * 9'144 * 9'144, 10'000L * 10'000 * 10'000)
	GENERATE_NSTD_FROM_FRACTION(AcreFeet, SI::CubicMeters, 9'144L * 9'144 * 4840 * 9'144, 10'000L * 10'000 * 10'000L * 3)
	GENERATE_NSTD_FROM_FRACTION(BoardFeet, SI::CubicMeters, 9'144L * 9'144 * 9'144, 10'000L * 3 * 10'000 * 3 * 10'000 * 3 * 12)
	GENERATE_NSTD_FROM_FRACTION(Cords, SI::CubicMeters, 9'144L * 9'144 * 9'144 * 128, 10'000L * 3 * 10'000 * 3 * 10'000 * 3)

	GENERATE_NSTD_FROM_FRACTION(UsMinims, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L * 128 * 8 * 60)
	GENERATE_NSTD_FROM_FRACTION(UsFluidDrams, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L * 128 * 8)
	GENERATE_NSTD_FROM_FRACTION(UsFluidOunces, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L * 128)
	GENERATE_NSTD_FROM_FRACTION(UsGills, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L * 32)
	GENERATE_NSTD_FROM_FRACTION(UsCups, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L * 16)
	GENERATE_NSTD_FROM_FRACTION(UsMeasures, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L * 16)
	GENERATE_NSTD_FROM_FRACTION(UsLiquidPints, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L * 8)
	GENERATE_NSTD_FROM_FRACTION(UsLiquidQuarts, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L * 4)
	GENERATE_NSTD_FROM_FRACTION(UsGallons, SI::CubicMeters, 3'785'411'784L, 1'000'000'000'000L)
	GENERATE_NSTD_FROM_FRACTION(PetroleumBarrels, SI::CubicMeters, 3'785'411'784L * 42, 1'000'000'000'000L)

	GENERATE_NSTD_FROM_FRACTION(UsDryPints, SI::CubicMeters, 2'150'420L * 16'387'064, 1'000L * 1'000'000'000'000 * 64)
	GENERATE_NSTD_FROM_FRACTION(UsDryQuarts, SI::CubicMeters, 2'150'420L * 16'387'064, 1'000L * 1'000'000'000'000 * 32)
	GENERATE_NSTD_FROM_FRACTION(UsPecks, SI::CubicMeters, 2'150'420L * 16'387'064, 1'000L * 1'000'000'000'000 * 4)
	GENERATE_NSTD_FROM_FRACTION(UsBushels, SI::CubicMeters, 2'150'420L * 16'387'064, 1'000L * 1'000'000'000'000)

	GENERATE_NSTD_FROM_FRACTION(ImperialFluidMinims, SI::CubicMeters, 4'546'090L, 1'000'000'000L * 8 * 20 * 8 * 60)
	GENERATE_NSTD_FROM_FRACTION(ImperialFluidDrachms, SI::CubicMeters, 4'546'090L, 1'000'000'000L * 8 * 20 * 8)
	GENERATE_NSTD_FROM_FRACTION(ImperialFluidOunces, SI::CubicMeters, 4'546'090L, 1'000'000'000L * 8 * 20)
	GENERATE_NSTD_FROM_FRACTION(ImperialGills, SI::CubicMeters, 4'546'090L, 1'000'000'000L * 32)
	GENERATE_NSTD_FROM_FRACTION(ImperialPints, SI::CubicMeters, 4'546'090L, 1'000'000'000L * 8)
	GENERATE_NSTD_FROM_FRACTION(ImperialQuarts, SI::CubicMeters, 4'546'090L, 1'000'000'000L * 4)
	GENERATE_NSTD_FROM_FRACTION(ImperialGallons, SI::CubicMeters, 4'546'090L, 1'000'000'000L)
	...
	GENERATE_NSTD_FROM_FRACTION(ThCalories, SI::Joules, 4'184L, 1'000L)
	GENERATE_NSTD_FROM_FRACTION(ItCalories, SI::Joules, 41'868L, 10'000L)
	GENERATE_NSTD_FROM_FRACTION(ThBtu, SI::Joules, 4'184L * 453'592'370 * 5, 1'000L * 1'000'000 * 9)
	GENERATE_NSTD_FROM_FRACTION(ItBtu, SI::Joules, 41'868L * 453'592'370 * 5, 10'000L * 1'000'000 * 9)
